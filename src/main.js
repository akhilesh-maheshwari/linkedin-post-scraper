...
try {
  ...
  const commentsLimit      = input.commentsLimit      || 0;
  const reactionsLimit     = input.reactionsLimit     || 0;

  const serviceName        = 'LinkedIn Post Scraper';
  const serviceOption1     = 'linkedin-post';
  const requestSource      = 'LinkedIn_Post_Scraper_AP';
  const boomerangInputUrl  = 'https://PLACEHOLDER.boomerangserver.co.in/webhook/post-scraper';
  const boomerangStatUrl   = 'https://PLACEHOLDER.boomerangserver.co.in/webhook/post-scraper-stats';
  ...
  if (!postUrls.length)       throw new Error('At least one post URL or ID is required!');

  // ──────────────────────────────
  // 2. VALIDATE + CLEAN URLS/IDs
  // ──────────────────────────────
  ...
  const trialStore  = await Actor.openKeyValueStore('boomerang-free-trials-linkedin-post');
  ...
  while (batchJobs && batchJobs.length > 0) {
          ...
          await fetch(
            'https://frontend.boomerangserver.co.in/webhook/Status_and_output_universal',
            {
              ...
              body   : JSON.stringify({
                ...
                requestStatus : 'Error',
                driveInputLink,
                boomerangOutputUrl: `https://PLACEHOLDER.boomerangserver.co.in/webhook/post-scraper-output?request_id=${request_id}`,
                batch_number,
                request_unique_id,
                batchFolderId,
                service_option_1 : serviceOption1,
    ...
    for (const result of batchStatusResults) {
      ...
      const boomerangOutputUrl = `https://PLACEHOLDER.boomerangserver.co.in/webhook/post-scraper-output?request_id=${request_id}`;
      ...
      if (rowsPushed > 0) {
        ...
        if (isFirstTime && totalFreeUsed === 0) {
          await trialStore.setValue(userId, { usedAt: new Date().toISOString(), runId, service: serviceName, rowCount });
